...
The US GAAP Taxonomy is designed so that the majority of elements have a positive value. This rules tests whether the values for a given list of elements are negative. The rule excludes elements when a specified member is present which would allow the value to be negative.  

See file [DQC_0015_ListOfElements](https://github.com/susanyount-wf/dqc_us_rules/blob/release_1susan/docs/DQC_US_0015/DQC_0015_ListOfElements.xlsx) which provides a list of elements that should not be negative. The rule tests whether these elements have a negative value in an instance file.  The elements are listed in order of most commonly occurring errors. 

See file [DQC_0015_MemberExclusions](https://github.com/susanyount-wf/dqc_us_rules/blob/release_1susan/docs/DQC_US_0015/DQC_0015_MemberExclusions.xlsx) for a list of members that, when used with these elements, allow the value of the element to be negative.  The rule does not test these member/element combinations.

## Authoritative reference

SEC Staff Observations December 13, 2011, June 15, 2011, November 1, 2010, October 6, 2009  

The US GAAP taxonomy is designed so that the majority of elements should have a positive value. In some cases, if the value also has a specified member associated with it, the value may be negative.

If the value is shown with brackets in the HTML filing, the XBRL value should still be positive, and a negating label can be used which will present the value in brackets in the rendering of the XBRL filing. Because negating labels will not change the XBRL value in the filing, the SEC rendered version of the filing should not be used to determine whether the XBRL value was reported correctly.

## Problem solved by the rule

Filers continue to have incorrect negative amounts in files, which is inconsistent with the definitions of the elements used and the structure of the US GAAP Taxonomy. Incorrect values in a filing makes it difficult or impossible to use that data for automated analysis.
